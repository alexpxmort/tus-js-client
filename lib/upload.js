...
class BaseUpload {
  ...
  _startParallelUpload() {
    ...
    let req;
    // Wait until all partial uploads are finished and we can send the PUT request for
    // creating the final upload.
    Promise.all(uploads).then(() => {
      req = this._openRequest("PUT", this.options.endpoint);
  ...
  /**
   * Create a new upload using the creation extension by sending a PUT
  ...
  _createUpload() {
    ...
    let req = this._openRequest("PUT", this.options.endpoint);
  ...
  _performUpload() {
    ...
    // cases, you can tell tus-js-client to use a PUT request with the
    // X-HTTP-Method-Override header for simulating a PATCH request.
    if (this.options.overridePatchMethod) {
      req = this._openRequest("PUT", this.url);
